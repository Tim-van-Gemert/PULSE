<span class="fixed bottom-0 w-screen z-50">
        <div class="bg-white dark:bg-black  pb-6 ">
            <div class="flex items-center pt-1 pb-3  justify-center">
                <div class="border-t-2 dark:text-white  text-slate-300 w-64"></div>
            </div>

            <div class="flex justify-around ml-8  w-screen content-center">
                <div  href="index.php" class="flex-col flex items-center justify-center">
                

                    <a href="surveys.php" >
                        <img src="../Media/Footer/iconmonstr-idea-12-240.png" class="ml-1 dark:invert w-8">
                    <h5 class="items-center text-center text-sm dark:text-white text-slate-500 ">Surveys</h5>
                    </a>
                </div>

                <div  class="flex-col flex items-center justify-center">
                <a href="index.php">
                        <img src="../Media/Footer/iconmonstr-building-40-240.png" class="ml-1 dark:invert w-8">
                        <h5 class="items-center text-center text-sm dark:text-white text-slate-500 ">Home</h5>
                    </a>
                    
                </div>

                <div href="report.php" class="flex-col flex items-center justify-center">
                    <a href="report.php" >
                        <img src="../Media/Footer/iconmonstr-heart-filled-240.png" class="ml-1 dark:invert w-8">
                    <h5 class="items-center text-center text-sm dark:text-white text-slate-500 ">Report</h5>
                </div>
            </div>
        </div>
        <script src="app.js"></script>
        <script src="date.js"></script>
        <script src="tinyslide.js"></script>

    </span>